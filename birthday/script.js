...
function showMemoryNote() {
    const notes = [
        'Happy Birthday, Kofoworola! E.K Fashion Fusion’s gonna soar! ??',
        'Your dancing and singing light up June 5 like Ziba’s waves!',
        'You’re my Burna Boy vibe, my love—shine at Chunz forever! ??'
    ];
...
if (countdownElement) {
    const countdown = setInterval(() => {
        ...
        countdownElement.textContent = `Ziba Beach Party in ${hours}h ${minutes}m ${seconds}s! ??`;
        if (distance < 0) {
            clearInterval(countdown);
            countdownElement.textContent = 'Happy Birthday, Kofoworola! Time to Shine at Ziba! ??';
        }
    }, 1000);
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 0; i < 100; i++) {
        confetti.push({
            ...
            r: Math.random() * 4 + 1,
            d: Math.random() * 100,
            color: ['#FFD700', '#FFB6C1', '#DDA0DD'][Math.floor(Math.random() * 3)],
